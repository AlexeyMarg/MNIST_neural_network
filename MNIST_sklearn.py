...
def use_KNeighЬorsClassifier(X_train, X_test, y_train, y_test):
    kn_clf = KNeighborsClassifier(weights='distance', n_neighbors=50)
    print('\n\nStart fit KNeighborsClassifier\n\n')
    kn_clf.fit(X_train, y_train)
    y_train_cv = cross_val_score(kn_clf, X_train, y_train, cv=3)
    print('KNeighborsClassifier results for test data:')
    print('Cross validation score:')
    print(y_train_cv)
    print('\nTest data results')
    y_test_predict = kn_clf.predict(X_test)
    conf_matrix = confusion_matrix(y_test, y_test_predict)
    print('Confusion matrix:')
    print(conf_matrix)
    accuracy = accuracy_score(y_test, y_test_predict)
...
kn_clf = use_KNeighЬorsClassifier(X_train, X_test, y_train, y_test)
